...
如果页面没有特殊状态压力，允许省略 `status`，由系统默认将其视作当前可用页。
...
1. 优先使用一个单独的计数器文件或等价的单写入机制分配新 `id`
2. 如果当前环境没有计数器文件，才退回到扫描现有页面、取最大编号加一
3. 不要在可能并行写入的场景里把“扫描最大值 + 1”当作可靠方案
4. `index.md` 只是对照表，不是唯一 ID 真相源
5. 旧 ID 不回收
