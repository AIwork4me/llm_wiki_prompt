...
请把下面这些内容视为默认候选结构，而不是必须一次性补齐的空骨架。优先创建当前任务真正需要的部分：
...
是否需要 `raw/articles/`、`raw/papers/`、`pages/entities/` 这类子目录，请按当前领域决定；不要为了凑结构而制造不会用到的层级，也不要为了“完整”保留空分区。

## 默认页面类型

默认可以从下面 6 类页面开始，但只有在当前资料和查询压力真的需要时才创建对应类型：
...
4. 根据资料密度和复用价值，创建或更新少量高信号页面；宁可少而准，不要批量造 stub，也不要预建“将来可能会用到”的页面。
...
4. 回写时采用 `draft -> promote` 两段式：
   ...
   - 只有在来源充分、结构整理完成、边界写清，并经过独立 sanity check 或明确的人类确认后，才提升为 `stable` 或 `contested`
